...
# Kopiert aus ltbbot und auf dbot angepasst
import ccxt
import pandas as pd
...
class Exchange:
    ...
    def close_position(self, symbol: str, side: Optional[str] = None):
        ...
        try:
            ...
            if amount is None:
                logger.error(f"Konnte Positionsgröße nicht lesen: {position}")
                return None
    ...
    def place_trigger_market_order(self, symbol: str, side: str, amount: float, trigger_price: float, reduce: bool = False, params={}):
        ...
        try:
            ...
            logger.error(f"Fehler beim Platzieren der Trigger Market Order für {symbol}: {e}", exc_info=True)
            raise e
    ...
    def place_trailing_stop_order(self, symbol, side, amount, activation_price, callback_rate_decimal, params={}):
        ...
        try:
            ...
            logger.info(f"Sende TSL-Aufruf: private_mix_post_plan_place_plan mit Params: {api_params}")
            ...
            logger.error(f"Kritischer Fehler beim Aufruf der TSL-Methode: {e}", exc_info=True)
            raise e
