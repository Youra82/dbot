# /root/dbot/src/dbot/strategy/run.py
import os
import sys
import json
import logging
...
def load_config(symbol, timeframe, use_macd_filter):
    ...
    suffix = "" if not use_macd_filter else "_alt"
    ...
    # Füge den automatisch bestimmten HTF zur Config hinzu
...
def run_for_account(account, telegram_config, params, model, scaler, logger):
    ...
    try:
        ...
        logger.info(f"--- Starte DBot (Physics) für {symbol} ({timeframe}) mit MTF-Bias von {htf} ---")
...
def main():
    parser = argparse.ArgumentParser(description="DBot Physics Trading-Skript")
    ...
    try:
        ...
        logger.critical(f"Unerwarteter Initialisierungs-Fehler: {e}", exc_info=True)
        sys.exit(1)

    # Hauptschleife
    while True:
        for account in accounts_to_run:
            if account.get('test_mode', False):
                logger.info(f"Account '{account.get('name', 'Standard')}' ist im TEST-MODUS. Überspringe...")
                continue
        ...
        logger.info(f"Schlafe für {sleep_time} Sekunden...")
